...
EGifPutCodeNext(GifFileType *GifFile, const GifByteType *CodeBlock)
{
    ...
    if (CodeBlock != NULL) {
        if ((long)InternalWrite(GifFile, CodeBlock, CodeBlock[0] + 1)
               != (long)(CodeBlock[0] + 1)) {
            GifFile->Error = E_GIF_ERR_WRITE_FAILED;
            return GIF_ERROR;
        }
                   ...
                   int c)
{
    if (c == FLUSH_OUTPUT) {
        ...
        if (Buf[0] != 0
            && (long)InternalWrite(GifFile, Buf, Buf[0] + 1) != (long)(Buf[0] + 1)) {
            GifFile->Error = E_GIF_ERR_WRITE_FAILED;
            return GIF_ERROR;
        }
        ...
        if (Buf[0] == 255) {
            ...
            if ((long)InternalWrite(GifFile, Buf, Buf[0] + 1) != (long)(Buf[0] + 1)) {
                GifFile->Error = E_GIF_ERR_WRITE_FAILED;
                return GIF_ERROR;
            }
            Buf[0] = 0;
